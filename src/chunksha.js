...
function ChunkSha(key, debug, maxChunkSize) {
  ...
  if (key !== undefined) {
    // keyA = toUTF8Array(key)
    // if (keyA.length > this.BLOCK_SIZE) {
    ...
    if (!(key instanceof ArrayBuffer)) {
      throw new Error('key has to be an instance of ArrayBuffer');
    }
    var key8 = new Int8Array(key);
    var keyBlock8 = new Int8Array(this.BLOCK_SIZE);
      
    if (key8.length > this.BLOCK_SIZE) {
      var instance = new ChunkSha();
      instance.update(key8.buffer);
      var raw = instance.get();
      var raw8 = new Int8Array(raw);
      if (this.endianness === "little endian") {
        var rawView = new DataView(raw);
        rawView.setInt32(0, rawView.getInt32(0, true));
        rawView.setInt32(4, rawView.getInt32(4, true));
        rawView.setInt32(8, rawView.getInt32(8, true));
        rawView.setInt32(12, rawView.getInt32(12, true));
        rawView.setInt32(16, rawView.getInt32(16, true));
      }
      keyBlock8.set(raw8);
      //key = hash(key) // keys longer than blocksize are shortened
    } else if (key8.length <= this.BLOCK_SIZE) {
      // keys shorter than blocksize are zero-padded (where ∥ is concatenation)
      // write key into array
      keyBlock8.set(key8);
      // no need to right pad with zeros as Int8Array are initialized with zeros
    }

    // compute o_key_pad 
    var i_pad_key8 = new Int8Array(this.BLOCK_SIZE);
    this.o_pad_key8 = new Int8Array(this.BLOCK_SIZE);
    for (var i = 0; i < this.BLOCK_SIZE; i++) {
      i_pad_key8[i] = keyBlock8[i] ^ 0x36;
      this.o_pad_key8[i] = keyBlock8[i] ^ 0x5c;
    }
    // here immediatelly process first block 
    this.update(i_pad_key8.buffer)
    this.hmac = true;
  }
}
...
ChunkSha.prototype.get = function () {
  ...
  var raw = this.heap.slice(0, 20);
  
  if (this.hmac) {
    var a = new ArrayBuffer(this.BLOCK_SIZE + 20);
    var a8 = new Int8Array(a);
    var raw8 = new Int8Array(raw);
    //here maybe flip the bytes if little-endian
    if (this.endianness === "little endian") {
      var rawView = new DataView(raw);
      rawView.setInt32(0, rawView.getInt32(0, true));
      rawView.setInt32(4, rawView.getInt32(4, true));
    ...
    a8.set(this.o_pad_key8);
    a8.set(raw8, this.BLOCK_SIZE);
...
ChunkSha.prototype.getHex = function () {
  ...
  if (!this.hexResult) {
    this.hexResult = hex(this.result);
  }
...
function handleFile (data, callback) {
  ...
  if (data.file && data.key) {
    chunkSha = new ChunkSha(data.key);
  }   

  var totalParts = Math.ceil(data.file.size/chunkSize);
  if (typeof FileReaderSync !== 'undefined') {
    // loop
    // Aug 2016 - Chrome, Firefox, Safari, Opera 
    callback({name: 'filereader', data: 'sync'});
    for (var n = 0; n < totalParts; n++) {
      readSync(chunkSha, data.file, n, chunkSize, totalParts, start, callback);
    }
  } else {
    ...
    readAsync(chunkSha, data.file, 0, chunkSize, totalParts, start, callback);
  }
}
